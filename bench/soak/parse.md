...
_Generated 2026-06-28 06:45 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-parse

_repo: https://github.com/r1chardj0n3s/parse · issue: There is a subtle edge-case bug in parse causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | decorator, with_pattern | 30,243→2,732 (11× smaller) | 161 sym idx (2.1s) · 10 recalled | 8s | $0.058 | ok |
| Cursor | composer-2.5-fast | parse/__init__.py | 211,642 | — | 474s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | parse/__init__.py, tests/test_findall.py | 1,487,282 | — | 127s | $0.453 est | ok |
