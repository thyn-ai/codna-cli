...
_Generated 2026-06-28 09:54 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-schema

_repo: https://github.com/keleshev/schema · issue: There is a subtle edge-case bug in schema causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | SchemaError, __init__ | 41,237→1,598 (26× smaller) | 182 sym idx (2.2s) · 10 recalled | 6s | $0.024 | ok |
| Cursor | composer-2.5-fast | schema/__init__.py | 101,743 | — | 106s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | schema/__init__.py, test_schema.py | 1,957,155 | — | 155s | $0.540 est | ok |
