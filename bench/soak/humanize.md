...
_Generated 2026-06-28 03:18 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-humanize

_repo: https://github.com/jmoiron/humanize · issue: naturaltime is off by one for an event exactly one minute ago_

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | nd_nomonths, test_date_and_delta | 23,463→598 (39× smaller) | 83 sym idx (1.3s) · 10 recalled | 7s | $0.018 | ok |
| Cursor | composer-2.5-fast | src/humanize/time.py | 120,090 | — | 224s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | src/humanize/time.py, tests/test_time.py | 1,198,288 | — | 162s | $0.407 est | ok |
