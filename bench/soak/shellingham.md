...
_Generated 2026-06-28 09:12 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-shellingham

_repo: https://github.com/sarugaku/shellingham · issue: There is a subtle edge-case bug in shellingham causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | _get_login_shell, _iter_process_parents | 8,861→2,530 (4× smaller) | 35 sym idx (0.8s) · 10 recalled | 7s | $0.046 | ok |
| Cursor | composer-2.5-fast | src/shellingham/posix/__init__.py | 152,020 | — | 210s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | src/shellingham/posix/__init__.py, tests/test_posix.py | 1,042,379 | — | 129s | $0.378 est | ok |
