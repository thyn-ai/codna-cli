...
_Generated 2026-06-28 06:11 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-jmespath.py

_repo: https://github.com/jmespath/jmespath.py · issue: There is a subtle edge-case bug in jmespath.py causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | _is_comparable, _is_special_number_case | 70,158→5,311 (13× smaller) | 319 sym idx (3.1s) · 10 recalled | 7s | $0.040 | ok |
| Cursor | composer-2.5-fast | jmespath/functions.py | 173,976 | — | 205s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | jmespath/visitor.py, tests/test_search.py | 1,214,051 | — | 135s | $0.389 est | ok |
