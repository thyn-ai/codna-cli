...
_Generated 2026-06-28 07:30 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-sniffio

_repo: https://github.com/python-trio/sniffio · issue: There is a subtle edge-case bug in sniffio causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | current_async_library | 7,883→2,152 (4× smaller) | 8 sym idx (1.1s) · 8 recalled | 7s | $0.042 | ok |
| Cursor | composer-2.5-fast | sniffio/_impl.py | 124,424 | — | 197s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | sniffio/_impl.py, sniffio/_tests/test_sniffio.py | 507,895 | — | 100s | $0.275 est | ok |
