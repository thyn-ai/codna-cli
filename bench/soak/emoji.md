...
_Generated 2026-06-28 05:42 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-emoji

_repo: https://github.com/carpedm20/emoji · issue: There is a subtle edge-case bug in emoji causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | get_emoji_by_name | 1,192,372→5,568 (214× smaller) | 151 sym idx (2.1s) · 10 recalled | 7s | $0.083 | ok |
| Cursor | composer-2.5-fast | emoji/tokenizer.py | 65,507 | — | 290s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | emoji/core.py, tests/test_core.py | 1,393,809 | — | 161s | $0.500 est | ok |
