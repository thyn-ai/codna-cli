...
_Generated 2026-06-28 05:54 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-furl

_repo: https://github.com/gruns/furl · issue: There is a subtle edge-case bug in furl causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | lget | 55,797→1,965 (28× smaller) | 248 sym idx (2.3s) · 10 recalled | 6s | $0.022 | ok |
| Cursor | composer-2.5-fast | furl/furl.py | 98,871 | — | 360s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | furl/furl.py, tests/test_furl.py | 1,520,101 | — | 119s | $0.479 est | ok |
