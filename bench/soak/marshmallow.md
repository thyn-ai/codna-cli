...
_Generated 2026-06-28 06:19 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-marshmallow

_repo: https://github.com/marshmallow-code/marshmallow · issue: There is a subtle edge-case bug in marshmallow causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | Field, get_value | 231,036→5,790 (40× smaller) | 1155 sym idx (9.4s) · 10 recalled | 12s | $0.115 | ok |
| Cursor | composer-2.5-fast | src/marshmallow/schema.py | 55,220 | — | 508s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | src/marshmallow/schema.py, tests/test_schema.py | 2,011,819 | — | 144s | $0.595 est | ok |
