...
_Generated 2026-06-28 06:51 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-platformdirs

_repo: https://github.com/platformdirs/platformdirs · issue: There is a subtle edge-case bug in platformdirs causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | _set_platform_dir_class | 84,328→5,961 (14× smaller) | 430 sym idx (3.8s) · 10 recalled | 8s | $0.074 | ok |
| Cursor | composer-2.5-fast | src/platformdirs/api.py | 162,223 | — | 126s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | src/platformdirs/api.py, tests/test_unix.py | 2,406,763 | — | 133s | $0.548 est | ok |
