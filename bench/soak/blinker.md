...
_Generated 2026-06-28 09:47 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-blinker

_repo: https://github.com/pallets-eco/blinker · issue: There is a subtle edge-case bug in blinker causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | make_id | 18,406→1,962 (9× smaller) | 63 sym idx (1.2s) · 10 recalled | 10s | $0.054 | ok |
| Cursor | composer-2.5-fast | src/blinker/base.py | 50,706 | — | 122s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | src/blinker/base.py, tests/test_context.py | 755,943 | — | 101s | $0.270 est | ok |
