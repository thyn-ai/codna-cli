...
_Generated 2026-06-28 10:13 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-dirty-equals

_repo: https://github.com/samuelcolvin/dirty-equals · issue: There is a subtle edge-case bug in dirty-equals causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | IsUUID, equals | 43,344→5,812 (7× smaller) | unavailable: For more information check: https://developer.mo | 5s | $0.043 | ok |
| Cursor | composer-2.5-fast | dirty_equals/_sequence.py | 106,058 | — | 241s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | dirty_equals/_sequence.py, tests/test_list_tuple.py | 1,814,996 | — | 143s | $0.543 est | ok |
