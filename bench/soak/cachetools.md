...
_Generated 2026-06-28 05:28 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-cachetools

_repo: https://github.com/tkem/cachetools · issue: There is a subtle edge-case bug in cachetools causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | _HashedTuple, __add__, __hash__ | 62,556→3,814 (16× smaller) | 390 sym idx (3.8s) · 10 recalled | 9s | $0.038 | ok |
| Cursor | composer-2.5-fast | src/cachetools/__init__.py | 168,283 | — | 187s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | src/cachetools/__init__.py, tests/test_cache.py | 1,754,012 | — | 163s | $0.611 est | ok |
