...
_Generated 2026-06-28 08:56 UTC · 1 repo(s) · each engine runs the same issue headless on its own checkout, no shared infra._

## clone-yarl

_repo: https://github.com/aio-libs/yarl · issue: There is a subtle edge-case bug in yarl causing incorrect behavior in a common code path; locate the responsible function and fix it with a minimal change._

| Engine | Model | Localized | Context tokens in | Memory (Telys) | Time | Cost | Status |
|---|---|---|--:|---|--:|--:|---|
| Codna +Telys | gpt-5.4 | normalize_path_segments | 165,408→4,670 (35× smaller) | 948 sym idx (9.5s) · 10 recalled | 6s | $0.038 | ok |
| Cursor | composer-2.5-fast | yarl/_url.py | 87,063 | — | 249s | Composer (subscription — no per-token rate) | ok |
| Codex | gpt-5.4 | tests/test_url_query.py, yarl/_parse.py | 2,520,886 | — | 161s | $0.686 est | ok |
